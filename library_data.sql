...
CREATE PROCEDURE GetBooksByMemberName(IN memberName VARCHAR(100))
BEGIN
    DECLARE memberId INT;

    -- Get MemberID using name
    SELECT MemberID INTO memberId
    FROM Members
    WHERE Name = memberName;

    -- Show borrowed book titles
    SELECT B.Title
    FROM Borrowing BR
...
CALL GetBooksByMemberName('Ravi Kumar');
